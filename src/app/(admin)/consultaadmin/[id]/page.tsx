...
export default function ConsultaContratado() {
  ...
  const { register, reset, watch } = useForm<ContratadoData>()
    ...
    async function getContratado() {
      try {
        ...
        if (response.ok) {
          ...
          const formattedDatedtnasc = `${dtnasc.getDate().toString().padStart(2, '0')}/${(dtnasc.getMonth() + 1).toString().padStart(2, '0')}/${dtnasc.getFullYear()}`
      ...
      <div className="grid grid-cols-1 gap-6 max-w-4xl mx-auto bg-gray-100 p-6 rounded-lg shadow-lg">
        <fieldset className="border border-gray-300 rounded-md p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
            ...
            <div className="sm:col-span-1">
              <label htmlFor="atuacao" className="block mb-2 text-sm font-medium text-gray-800">Área de atuação</label>
              <input
                {...register("atuacao")}
                type="text"
                id="atuacao"
